...
- **Task fit** (`config/routing.yaml`, v1.2): a 0–1 rating per task type for each provider or model. `balanced` moves from the cheapest capable model to a better fit when fit improves by ≥ `min_edge` (0.1) and cost is ≤ `balanced_price_band` (3×); `quality` sorts by fit within the strongest tier; `cheapest` ignores fit. Custom and connected endpoints get `custom_strength` (0.8, unknown quality) unless the user rates them (`quality` 0–1 on a request's custom model, or on a saved endpoint via the account page / `PATCH /v1/providers/{id}`, migration 0005), and the band is measured from at least the cheapest built-in model of the tier (`band_floor`), so a near-free custom model can't shut out every better fit. On the eval set balanced now picks Gemini 3.8 Flash 46%, Claude Sonnet 5 28%, GPT-6 Luna 25%.
